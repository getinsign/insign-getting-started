...
public class InSignExternAbortHandler extends MyHttpHandler implements HttpHandler {

	public InSignExternAbortHandler(Map<String, InSignSessionHandle> inSignSessionHandles, InSignAdapter<?> adapter) {
	...
	public void handle(final HttpExchange exchange) throws IOException {
		try {
			...
			final JSONBasicResult result = InSignAdapterDemo.adapter.abortExternal(sessionHandle);
			String response = "<html>";
			response += "<meta charset=\"UTF-8\"/>";
			response += "inSign ExternalAbortResult: " + InSignAdapterDemo.adapter.logObject(result);
